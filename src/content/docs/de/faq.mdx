...
description: "Beantwortet häufige Fragen zur Konfiguration von evcc, zur Fehlersuche, zu unerwartetem Ladeverhalten, zum Zurücksetzen des Passworts und zur PV-Prognose."
...
Stattdessen wird im Lademodus **Smart** mit der PV-Erzeugungsleistung gearbeitet.
...
In den Wintermonaten ist die PV Erzeugung meist unterhalb oder im Bereich der [Mindestleistung](/de/features/solar-charging#charging-start).
Um dennoch möglichst viel eigenen Strom ins Fahrzeug zu laden, kann man folgende "Tricks" anwenden:

#### mittels `residualpower`

In der Konfiguration unter [`site`](/de/reference/configuration/site) setzt man [`residualPower`](/de/reference/configuration/site#residualpower) auf einen negativen Wert. Dieser bestimmt dann wieviel Netzbezug im Smart-Modus noch erlaubt ist. Eine Änderung ist per API möglich.

**Beispiel**:

```yaml
site:
  residualPower: -1000 # 1000W Netzbezug im Smart-Modus erlaubt
```

"Nachteil" dieser Lösung ist, dass auch bei ausreichendem Überschuss dieser erlaubte Netzbezug ausgenutzt wird.

#### mittels Sonnenanteil-Regler

Stelle in den Einstellungen des Ladepunkts den Regler **Sonnenanteil** niedriger, damit nur ein Teil der Mindestladeleistung aus Solarstrom stammen muss.
Details findest du unter [Nicht genügend Überschuss?](/de/features/solar-charging#not-enough-surplus).
