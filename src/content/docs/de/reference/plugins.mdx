---
title: Plugins
description: "Plugins lesen und schreiben einzelne Werte per Modbus, HTTP, MQTT, Skript und Co. und bilden damit die Grundlage für benutzerdefinierte Geräte."
tableOfContents:
  maxHeadingLevel: 4
---
...
`uri` und `body` sind Go-Templates, die bei jeder Anfrage neu ausgewertet werden. Neben den [sprig-Funktionen](https://masterminds.github.io/sprig/) stehen `addDate` und `timeRound` zur Verfügung. Damit lässt sich z. B. eine Prognose-API immer für einen mitlaufenden Zeitraum abfragen:
...
**Beispiel mit Batteriesteuerung** (`batterymode` verwendet Werte 1=normal, 2=hold, 3=charge, 4=holdcharge, 5=discharge):
