---
title: Plugins
description: "Plugins read and write single values via Modbus, HTTP, MQTT, scripts and more, and form the building blocks of user-defined devices."
tableOfContents:
  maxHeadingLevel: 4
---
...
`uri` and `body` are Go templates that are evaluated on every request. Besides the [sprig functions](https://masterminds.github.io/sprig/), `addDate` and `timeRound` are available. This allows e.g. a forecast API to always be queried for a moving date range:

```yaml
forecast:
  source: http
  uri: https://api.example.org/forecast?from={{ now | date "2006-01-02" }}&to={{ addDate now 0 0 2 | date "2006-01-02" }}
```
...
**Example with battery control** (`batterymode` uses values 1=normal, 2=hold, 3=charge, 4=holdcharge, 5=discharge):
