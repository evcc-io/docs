---
title: "chargers"
description: "How to define chargers in evcc.yaml, which fields are required, and how to mark integrated devices, heating or smart switches."
sidebar:
  order: 4
---
...
Marks a device that keeps running in its own normal operation when "disabled".
The UI shows "Normal operation" instead of "Standby".
A request to increase power (e.g. on PV surplus or cheap grid power) is labelled "Boost".

Set this feature when your device keeps running in its regular mode without active control.
The typical use case is a heat pump; preconfigured heat pump types and matching templates already set the feature.
...
The UI hides the min/max current settings and the **Always charge** option for this loadpoint.
