...
| `batterymode` | `int` | no       | `battery` | Set charging mode directly (1: normal, 2: hold, 3: charge, 4: holdcharge, 5: discharge)                                                                           |
...
If `batterymode` is a [`switch`](/en/reference/plugins#switch) (optionally wrapped in a `watchdog`), the supported modes are taken from its `case` values.
