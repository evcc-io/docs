...
description: "Answers frequently asked questions about configuring evcc, tracking down errors, unexpected charging behaviour, password resets and the solar forecast."
...
:::note[There is no automatic migration.]
...
[`residualPower`](/en/reference/configuration/site#residualpower) config flag.
...
How to access the log is described in [Installation](/en/installation) under your respective installation method.
...
In the settings of the charging point, lower the **Solar Share** slider so that only part of the minimum charging power has to come from solar.
See [Not enough surplus?](/en/features/solar-charging#not-enough-surplus) for details.
...
This contains all configured devices, services, plans, charging sessions, etc.

**Restore:**
Restores a previously downloaded database.
A local backup (`evcc.db.bak`) is automatically created before overwriting.

:::caution[Caution]
All actions are password-protected.
Restoring will completely overwrite your current data.
:::

**Reset:**
You can delete individual areas of your database:
...
You can use various [pv forecasts](/en/tariffs#pv-forecast) in evcc to see the estimated remaining production for the current day and the upcoming days.
