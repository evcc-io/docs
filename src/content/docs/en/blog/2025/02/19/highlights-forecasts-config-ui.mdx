...
description: "Release highlights for evcc 0.200 from February 2025 with solar forecasts, repeating charging plans, SG Ready heat pumps and browser-based configuration."
authors: [naltatis]
...
We've had [CO₂-optimized](/en/features/co2) and [price-optimized](/en/features/dynamic-prices) charging for a while.
...
For this, there's a new `solar` field in the [tariff configuration](/en/tariffs).
...
The ability to establish a [minimum charge level](/en/features/limits#minimum-charge) immediately after plugging in has been around for a while.
Charging the vehicle efficiently and cleanly using a [charging plan](/en/features/plans) (departure time and target charge level) is also a long-standing feature.
...
Integration of heat generators was possible through methods like [relays, smart plugs](/en/smartswitches#heating) and the plugin interface ([example](https://github.com/naltatis/aton-ctrl)), and was being used.
...
<SgreadySvg
  alt="Diagram showing heat pump control via SG-Ready relay"
