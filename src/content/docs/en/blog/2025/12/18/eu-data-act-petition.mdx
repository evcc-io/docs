...
description: "In December 2025 we explained why evcc needs vehicle data, what the EU Data Act promises and how you can support the petition for open access to vehicle data."
authors: [naltatis]
...
"Mine works fine" – we hear that often.
The reality behind it: Most vehicle integrations in evcc are based on unofficial, undocumented APIs.
Developers spend their free time analysing how manufacturer apps communicate internally and build their integrations on that.

This works – but only until the manufacturer changes something in their systems.
Then the integration breaks without warning.
This has happened repeatedly in the past and will continue to happen.

The problem doesn't just affect evcc.
All open source solutions like Home Assistant face the same challenge.
As long as manufacturers don't provide official access, it remains a constant cat-and-mouse game with an uncertain outcome.

With official interfaces it would be different: documented, stable, reliable.
You could rely on your integration not suddenly stopping to work.
