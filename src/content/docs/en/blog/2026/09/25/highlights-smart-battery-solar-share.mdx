...
:::note[Update]
**26 September 2026:** The [Smart mode](#modes) is now explained in more detail, and the new [modes page](/en/features/modes) covers all modes.
The [optimizer section](#optimizer) now mentions that the optimizer requires a sponsor token.
:::

## New Modes: Smart and Always Charge \{#modes\}
...
**Off** and **Fast** are simple switches: the device stays off, or it runs at full power like a charger without evcc.
Everything evcc decides on its own happens in **Smart** mode:

- [Solar surplus charging](/en/features/solar-charging)
- Cheap or green grid charging with a [dynamic tariff](/en/features/dynamic-prices) or [CO₂ forecast](/en/features/co2)
- Priorities between [home battery](/en/features/battery), charging points and heaters
- [Charging plans](/en/features/plans)

**Smart** is also the mode the upcoming optimizer will work in.
...
The new [modes page](/en/features/modes) explains all modes and [Always charge](/en/features/modes#always-charge) in detail.
...
The optimizer requires a [sponsor token](/en/sponsorship).
The calculation runs on a stateless cloud service hosted by us that doesn't store any data.
You can also run the service yourself, see [technical background](/en/features/optimizer#technical).
