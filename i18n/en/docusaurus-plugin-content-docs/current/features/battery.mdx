...
<Screenshot
  ...
  caption="Screenshot of the main navigation."
/>
...
<Screenshot
  ...
  caption="Priority house and car"
/>

Example: You set the limit to 50%. This means that solar energy flows into the home battery first.
If the battery reaches a charge level of 50%, charging points that are in PV mode are released.
...
## Battery-assisted vehicle charging

In PV mode, evcc tries to control the charge so that only solar energy is used directly.
Discharging the home battery is avoided.
With the **Battery-assisted vehicle charging** function, you can explicitly release part of your home storage for charging the electric car.
...
<Screenshot
  ...
  caption="Battery-assisted vehicle charging"
/>

Example: You release the area above 75% for battery-assisted charging.
Your home storage has already filled up to 100% during the day.
You plug in your electric car in PV mode.
...
It then enables you to charge the car in the evenings, for example, when there is little sun.
Efficiency and conversion losses: In general, it is advisable to charge the car directly with solar energy if possible. The detour via the home battery always means conversion losses, which, depending on the system, can amount to [5-10%](https://solar.htw-berlin.de/studien/stromspeicher-inspektion-2024/).

### Automatic start

In PV mode, the charging process only starts when there is a surplus available.
...
You can change this behavior with the "starts automatically ..." option.

If you set the value here to "... when over 90%", for example, the charging process starts when the home battery has reached this level.
...
You can view the individual charge levels in the "House battery" dialog.

## Active battery control

The functions described above are compatible with all battery systems.
evcc simply adjusts the charging control of the electric car depending on the charge level of the house battery.
...
However, you don't want the house battery to be completely discharged as a result.
...
However, the house battery will try to cover the charging needs.

The cheap night-time electricity will only be used when the storage is empty.
If your system supports active battery control, you can activate the "Discharge lock during fast charging" option in the "Battery settings" dialog.
This puts the house battery into a locked state when a fast charging process is active.
This can be triggered by fast mode, smart grid charging, charging planning or minimum charging.

The locked state applies for the period of fast charging.
The house battery is not discharged during this time.
...
Under [Home installation](../devices/meters) you can see on the day **Active battery control** whether your system is supported.
Some inverters also have additional configuration options such as upper charging and discharging limits, which may need to be adjusted.
