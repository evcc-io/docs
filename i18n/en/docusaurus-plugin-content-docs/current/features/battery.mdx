...
However, as long as it has some energy in it, the home battery will try to cover the charging demand.
The cheap night-time electricity will only be used when the valuable storage has been used up.
...
:::info German regulation §14a EnWG
During active [power reduction from external control](./external-control), grid charging is automatically paused and battery discharge is locked ("hold" mode).
...
Under [PV, Battery, Grid, Meter](../devices/meters) you can see on the **Active battery control** tag whether your system supports battery control and if `maxsoc` is available.
