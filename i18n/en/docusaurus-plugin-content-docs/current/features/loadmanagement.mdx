...
:::info Note
External control can be used to implement control requests from the grid operator.
For more information, see [External Control](./external-control).
:::
...
- Home battery [grid charging](./battery#gridcharge) is not considered. When the home battery actively charges from the grid, this consumption is not included in load management calculations. However, during active [§14a power reduction](./14a-enwg-steuve) signals, grid charging is automatically paused.
