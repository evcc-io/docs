...
This configuration only configures a main circuit. All charging points without an explicit circuit assignment are assigned to this main circuit. The circuit has a maxcurrent of 63A if there are other consumers like an oven/heat pump (this requires a meter) using a total of 50A the charger will only be allowed to use 13A.
...
**Important:** There must always be a main circuit. This has no `parent` property. All charging points without an explicit circuit assignment are assigned to this main circuit.
