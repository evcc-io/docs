...
<TabItem value="mqtt" label="modbus">

If the control box or gateway communicate the power limit via modbus:
...
hems:
  ...
  limit: 
    source: modbus  # Details for these parameters can be found in the evcc-modbus documentation 
    uri: 192.168.179.200:4703  # Example for a relay connected to the 2nd S0-Counter at a cfos power brain solar wallbox
    id: 3  # modbus slave-ID of the device 
    timeout: 5s  
    register:
      type: holding  
      decode: uint16  
