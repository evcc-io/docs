...
All charging points are automatically assigned to the internal `lpc` circuit and reduced accordingly when a control request is received.
...
If you don't define it manually, it will be automatically created when HEMS is configured and inserted above the highest circuit.
:::
...
This applies e.g. to heat pumps or battery storage systems that are connected via EEBus or configured as meters with a switching output.
...
:::note Consumer Control
This feature is currently only implemented for EEBus meters with LPC.
