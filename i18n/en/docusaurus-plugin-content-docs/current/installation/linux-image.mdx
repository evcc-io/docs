...
Go to the **[evcc Linux Images](https://github.com/evcc-io/images/releases)** and download the latest version for Raspberry Pi (`armbian_evcc-{version}_rpi.img.zip`).
...
| Cockpit     | `https://evcc.local:9090/` (initial password: `admin`) |
| SSH         | `ssh admin@evcc.local` (initial password: `admin`)     |

The services use self-signed certificates.
You need to accept the certificate warning in your browser, or disable certificate verification in your wallbox's OCPP configuration.
If you have compatibility issues, you can use the unencrypted alternatives:
