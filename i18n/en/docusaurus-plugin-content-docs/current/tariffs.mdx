...
Useful for "Tafiff model 3" where tariff is different between summer and winter and furthermore tariff changes during the day.
...
```
In this example, the winter period is defined as October to March and summer from April to September. 
In each season the tariff for high load, low load and peak load is defined. This value is then added to the price before tax is calculated
The above example is working with Dynamic Electricyty Prices 
