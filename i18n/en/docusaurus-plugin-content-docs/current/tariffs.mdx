...
### Exchange Prices with Season and Time Component
Useful when tariff is different between summer and winter and furthermore tariff changes during the day with peak and off-peak tariffs.

The tariff is defined in time-slots:
"low load tariff" from 00 to 06,  "high load tariff" from 06 to 17,  "peak load tariff" from 17 to 21 and "high load tariff" from 21 to 24:
...
```
This is an example of a tariff that vary throughout the year (summer, winter). 
In addition, there are high- and low-price zones throughout the day. The tariff is divided into time slots: "low load tariff" from midnight to 6 a.m., "high load tariff" from 6 a.m. to 5 p.m., "peak load tariff" from 5 p.m. to 9 p.m., and again "high load tariff" from 9 p.m. to midnight.
The total price is calculated from the exchange price, fees and taxes
The above example is working with dynamic electricity prices. 
