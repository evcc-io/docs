...
Tariff plugins are updated once per hour.
The update frequency of integrated tariff providers depends on the respective provider and their rate limits.
For some providers, the frequency can be adjusted via the `interval` parameter.

evcc automatically caches fetched tariff data.
On restart, cached data is used if it's still valid.
This prevents hitting API rate limits during frequent restarts.
...
<DeviceConfig code={`tariffs:
    grid:
      ...
      max: 0.39 # Maximum price, Highest price, typically in the evening (optional)`} advanced={`tariffs:
    grid:
      type: template
      template: demo-dynamic-grid
      min: 0.10 # Minimum price, Lowest price, typically at noon (can be negative) (optional)
...
<DeviceConfig code={`tariffs:
    co2:
      type: template
      template: ned
      apiKey: # API Key, Create an app in https://ned.nl/nl/user/register and copy the key`}  />
