...
You can also combine the `months` condition with `days` and `hours`. The following is a more complex example to illustrate the possibilities of `zones`.
  ...
  grid:
    ...
    zones:
      ...
      - months: Jan-Mar,Oct-Dec # winter low tariff (at weekends from October to March inclusive)
        days: Sa,So
        hours: 22-0,0-6 # 22:00 to 05:59; note: 22-6 is not possible, as 00:00 is passed over
        price: 0.25 # CHF/kWh
```
