...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: ckw
      tariff: # Tariff name, home_dynamic (below 50 MWh/year) or business_dynamic (above 50 MWh/year) [home_dynamic, business_dynamic]`}
  advanced={`tariffs:
    grid:
      type: template
      template: ckw
      tariff: # Tariff name, home_dynamic (below 50 MWh/year) or business_dynamic (above 50 MWh/year) [home_dynamic, business_dynamic]
      charges: # Charge, Additional fixed charge per kWh (optional)
      tax: 0.081 # Tax, Additional percentage charge (e.g. 0.2 for 20%) (optional)
      formula: math.Max((price + charges) * (1 + tax), 0.0) # Formula, Individual formula for calculating the price (optional)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceFeatures features="Germany,Luxembourg,Austria,France,Netherlands,Belgium,Poland,Denmark" />
...
<DeviceFeatures features="Germany,Austria,Netherlands,Belgium,Sweden,Denmark" />

EPEX spot price predictions from epexpredictor.batzill.com
...
<DeviceConfig
  ...
  advanced={`tariffs:
    grid:
      ...
      template: epex-predictor
      region: DE # Region, [DE, AT, NL, BE, SE1, SE2, SE3, SE4, DK1, DK2]
      uri: https://epexpredictor.batzill.com # API endpoint (for self-hosted instances), HTTP(S) address (optional)
      charges: # Charge, Additional fixed charge per kWh (optional)
      tax: # Tax, Additional percentage charge (e.g. 0.2 for 20%) (optional)
      formula: math.Max((price + charges) * (1 + tax), 0.0) # Formula, Individual formula for calculating the price (optional)
      average: # Average by hour, optional`}
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      provider: # Energy provider, [anwb-energie, atoom-alliantie, budget-energie, coolblue-energie, easyenergy, eneco, engie, energie-vanons, energyzero, frank-energie, frank-energie-slim, hegg, innova, nextenergy, samsam, tibber, vandebron, zonneplan]
      price-interval: quarterly # Price interval, [hourly, quarterly]`}
  advanced={`tariffs:
    grid:
      type: template
      template: epexprijzen-nl
...
You can get the API key in the Octopus portal: [octopus.energy](https://octopus.energy/dashboard/new/accounts/personal-details/api-access)
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: octopus-de
      email: user@example.com # Email Address, The email address of your Octopus Energy account.
      password: secret # Password, The password of your Octopus Energy account.
      accountNumber: A-XX345678 # Account Number, Your Octopus Energy account number (e.g., A-12345678).`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceFeatures features="Poland" />
...
Price data from [mercatoelettrico.org](https://www.mercatoelettrico.org/it/). Often used for feeding into the grid.
...
Stekker.app spot prices in day-ahead market and 24h AI price Forecast
...
<DeviceFeatures features="Germany,Netherlands,Belgium,Denmark,France" />

Spot prices in day-ahead market plus 4 days prognosis [stroomprijsprognose.nl](https://stroomprijsprognose.nl)
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      currency: EUR # Currency, [EUR, DKK]
      horizon: 120 # Forecast horizon (h)`}
  advanced={`tariffs:
    grid:
      type: template
      template: stroomprijsprognose
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      price: # Default price, When no zone applies.
      zones: # Price zones`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      token: # Token
      zone: DE # Zone, see [api.electricitymap.org](https://api.electricitymap.org/v3/zones)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceFeatures features="Finland" />

CO₂ data for Finnish national grid from [Fingrid OpenData](https://www.fingrid.fi/en/electricity-market-information/real-time-co2-emissions-estimate/). Get your API key by registering an account at [https://data.fingrid.fi/instructions](https://data.fingrid.fi/instructions).
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      apiKey: # API Key, Get your API key by registering an account at [https://data.fingrid.fi/instructions](https://data.fingrid.fi/instructions), creating a new API key and enter it here.`}
  advanced={`tariffs:
    co2:
      type: template
      template: fingrid-co2
...
Regional emission data from [gruenstromindex.de](https://gruenstromindex.de)
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      zip: # ZIP code
      token: # Token, Token for accessing the API from [console.corrently.io](https://console.corrently.io/) (optional)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
Dutch Energy Production CO₂ data from [ned.nl](https://www.ned.nl). Provides forecasts for 12 hours ahead and is free of charge after registration.
...
<DeviceConfig
  code={`tariffs:
    co2:
      type: template
      template: ned
      apiKey: # API Key, Create an app in [ned.nl](https://ned.nl/nl/user/register) and copy the key`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    co2:
      type: template
      template: ngeso
      region: 1 # Region, Coarser than using a postcode. See [carbon-intensity.github.io](https://carbon-intensity.github.io/api-definitions/#region-list) (optional)
...
<DeviceFeatures features="Germany,Netherlands,Belgium,Denmark" />

5 days prognosis [stroomprijsprognose.nl](https://stroomprijsprognose.nl)
