...
<DeviceConfig
  ...
  advanced={`vehicles:
    - name: my_car
      ...
      maxPower: # Maximum vehicle charging power (W), Defines the maximum charging power of the vehicle. Helps improve charge plan accuracy when the vehicle typically uses less than the offered current or supports higher single-phase current compared to three-phase. The offered current of the loadpoint is not affected. (optional)
