...
Firmware requirements= Smart/Touch Pro from "M3W_3.11STP", Business from "M3W_3.09PTB". Charging pauses for two minutes during phase switching.
...
<DeviceConfig
  code={`chargers:
    - name: my_charger
      ...
      enabled: binary_sensor.charger_enabled # Enabled status sensor, Entity ID for enabled state (sensor or binary_sensor with on/off or true/false state)
