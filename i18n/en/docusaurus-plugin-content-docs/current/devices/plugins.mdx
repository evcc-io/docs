...
- Ensures timeouts are followed on updates
- The watchdog is stopped during the delay and restarted with the new value after it expires
- Useful when devices require a minimum wait time between mode transitions
- The delay is calculated based on time since last update
