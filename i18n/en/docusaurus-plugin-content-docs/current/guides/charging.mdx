...
Solar Excess is calculated by monitoring the feed-in power delivered back to the Grid. Excess calculation therefore does not function if this is not available, but you [can still use Solar mode if you have a PV meter](https://docs.evcc.io/en/docs/guides/meters#i-have-a-solar-installation-but-i-dont-have-a-readable-grid-connection-meter---can-i-still-use-evcc) and configure an assumed, constant consumption for the rest of your house.

Have a look at the [How does Solar mode work - a look into the innards](./solar-mode-innards) guide for an in-depth explanation of how solar mode works and the parameters defining its behavior.
