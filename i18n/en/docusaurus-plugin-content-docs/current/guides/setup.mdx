...
:::caution
A lot of individual settings, knobs, and dials can be played with here. The default parameters are based on lots of experience from many users, and in general, should work well for you.

**If you do start making changes, remember the golden rule:**

**Since parameters sometimes influence each other, it is STRONGLY encouraged to only make changes to one setting at at time, and test the effects of each change before making further modifications.**
:::
...
Yaml linters such as (https://onlineyamltools.com/validate-yaml) can be super useful, and are worth checking to find simple mistakes.
...
### How do I create a log fire for error analysis?
...
### Error: `/tmp/evcc: operation not permitted` and/or `bind: address already in use`
