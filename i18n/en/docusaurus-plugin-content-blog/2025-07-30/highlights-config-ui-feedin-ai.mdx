...
In this mode, devices are simulated with [simple mock scripts](https://github.com/evcc-io/evcc/blob/master/cmd/demo.yaml).
...
These `custom` devices previously had to be configured via `evcc.yaml`.
...
If you have multiple PV or battery systems or charging points configured, you can now expand them in the energy flow view to see more details.
Devices created via the configuration interface can be given a name for this purpose.

:::hint
Naming meters is not possible via `evcc.yaml` for technical reasons.
:::
