...
<DeviceConfig code={`chargers:
  ...
  <div className="device-detail-icon">
    <evcc-icon
      ...
      size="400px"
      alt="Home Assistant Schalter"
      className="device-detail-icon-img"
    ></evcc-icon>
  </div>
</div>

## Templateparameter `homeassistant-switch`

| Name | Erklärung | Wert | Optional |
|-----------|-----------|------|----------|
| Basis-URL der Home Assistant Instanz<br/>`baseurl` |  | Beispiel: `http://homeassistant.local:8123` | ja |
| Long-Lived Access Token<br/>`token` | aus Home Assistant Profil |  | ja |
| Entity ID des schaltbaren Geräts<br/>`switchentity` |  | Beispiel: `switch.smartsocket` | ja |
| Entity ID für Leistungsmessung<br/>`powerentity` |  | Beispiel: `sensor.smartsocket_power` | nein |
