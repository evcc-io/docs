...
| Vehicle Identification Number<br/>`vin` | If you own multiple vehicles from the same manufacturer | Beispiel: `ZFAE...` | no |
| Title<br/>`title` | Will be displayed in the user interface |  | no |
| Battery capacity<br/>`capacity` |  | Beispiel: `50`<br/>Einheit: kWh | no |
| PIN<br/>`pin` | Required for evcc to wake up the vehicle for charging and to refresh the SoC while charging. When connected to TWC3, used to start/stop charging. |  | no |
