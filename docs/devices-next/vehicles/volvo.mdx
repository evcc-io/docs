...
| VCC API Key<br/>`vccapikey` | from \[Volvo Developer App\](https://developer.volvocars.com/). |  | yes |
| Client ID<br/>`clientId` | from \[Volvo Developer App\](https://developer.volvocars.com/). |  | yes |
| Client Secret<br/>`clientSecret` | from \[Volvo Developer App\](https://developer.volvocars.com/). |  | yes |
| Redirect URI<br/>`redirectUri` | Redirect URI of your evcc instance, format: `http://evcc.local:7070/oauth/callback`. Must match the redirect URI set in your Volvo Developer App. |  | yes |
| Vehicle Identification Number<br/>`vin` | If you own multiple vehicles from the same manufacturer | Beispiel: `WF0FXX...` | no |
