...
### Wie kann ich meine Konfiguration und Einstellungen zurücksetzen? {#config-reset}

In der evcc UI gibt es unter **Einstellungen** → **System** → **Zurücksetzen** Optionen zum Zurücksetzen verschiedener Daten.

**Ladevorgänge zurücksetzen:**
Löscht den Verlauf deiner Ladevorgänge.

**Konfiguration & Einstellungen zurücksetzen:**
Löscht alle über die UI konfigurierten Geräte, Dienste, Pläne und weitere Einstellungen.
Außerdem werden zwischengespeicherte Daten (z. B. Tarifinformationen) und Zählerstände gelöscht.
...
Alternativ kannst du einzelne zwischengespeicherte Daten über die Kommandozeile löschen:
...
Als expeimentelles Feature haben wir auch die Möglichkeit eingebaut, die Vorhersagen an deine tatsächliche Produktion anzupassen.
