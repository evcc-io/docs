...
Generell kann evcc das Laden und Entladen der Hausbatterie nicht direkt beeinflussen. Das Hausbatteriemanagementsystem (BMS) entscheidet selbst wann es lädt. Hier gibt es üblicherweise folgende Szenarien:
...
1. **Hausbatterie wird geladen**:
  ...
  - Wenn das BMS andere Logiken eingebaut hat, z.B. um sich zu kalibrieren (z.b. E3/DC)
  - Und noch weitere
2. **Hausbatterie wird nicht geladen**:

  - Wenn die Hausbatterie voll ist, wird die Batterie nicht geladen.
  - Wenn das BMS die Fähigkeit von prognosebasiertem Laden (PV Prognose und ein historischen Verbrauch ist vorhanden um die Zukufnt abzuschätzen) hat, um nicht voll zu laden wenn vorraussichtlich nicht so viel Strom benötigt wird (z.b. SMA Home Manager 2.0)
  - Und noch weitere
...
2. **Lademodus `PV`**
  ...
  - Der Ladevorgang wird immer unterbrochen, wenn über deine definierte Dauer ([Standard: 1 Minute](/docs/reference/configuration/loadpoints#disable)) nicht genügend Strom für eine [Mindestleistung](/docs/guides/faq#ich-habe-pv-überschuss-aber-das-auto-lädt-nicht-warum) vorhanden ist.
  - Zwischen dem [Invervall des Regelzyklus](/docs/reference/configuration/interval) kann es jedoch zu Schwankungen kommen, so dass weniger Überschuss vorhanden ist als zum Regelzeitpunkt. D.h. falls hier zu wenig PV Überschuss vorhanden ist, muss der "fehlende Strom" irgendwo her kommen. Und wenn die Hausbatterie noch nicht "leer" ist, springt das BMS ein und zieht Strom aus der Hausbatterie um den Netzschluss Punkt auf 0 auszuregeln. Dies kann nicht verhindert werden!
  - Zum Zeitpunkt der Regelung in evcc, kann mit 
    ...
    - [bufferSoC](/docs/reference/configuration/site#buffersoc) die Hausbatterie oberhalb diesesdes definierten SoC eine Entladung igoriert und als PV Überschuss interpretiert. 
...
Als Optimierung kann über die Einstellung [`residualPower`](/docs/reference/configuration/site#residualpower) ein grober konstanter Hausverbrauch angegeben werden.
