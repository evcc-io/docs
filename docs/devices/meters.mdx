...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      host:  # Hostname
      port: 502 # Port
      batterytype: lv # Wechselrichter für LV- oder HV-Batterie, "hv" für eine Hochvolt-Batterie mit Nennspannung über 60V verwenden [lv, hv]
      maxdischargecurrent: 100 # Max Entladestrom, Maximaler Batterieentladestrom in Ampere (A) für Normalmodus. Typischer Bereich 50-350A abhängig von Batteriekapazität. Leistungsberechnung: P(W) = Strom(A) x Spannung(V). Beispiel mit 48V Batterie: 100A x 48V = 4800W (4,8kW).
      gridchargecurrent: 60 # Netzladestrom, Maximaler Batterieladestrom in Ampere (A) aus dem Netz für Lademodus. Typischer Bereich 30-350A abhängig von Batterie- und Wechselrichterkapazität. Leistungsberechnung: P(W) = Strom(A) x Spannung(V). Beispiel mit 48V Batterie: 60A x 48V = 2880W (2,9kW).`}
  advanced={`meters:
    - name: my_grid
      type: template
      template: deye-hybrid-3p
      ...
      maxdischargecurrent: 100 # Max Entladestrom, Maximaler Batterieentladestrom in Ampere (A) für Normalmodus. Typischer Bereich 50-350A abhängig von Batteriekapazität. Leistungsberechnung: P(W) = Strom(A) x Spannung(V). Beispiel mit 48V Batterie: 100A x 48V = 4800W (4,8kW).
      gridchargecurrent: 60 # Netzladestrom, Maximaler Batterieladestrom in Ampere (A) aus dem Netz für Lademodus. Typischer Bereich 30-350A abhängig von Batterie- und Wechselrichterkapazität. Leistungsberechnung: P(W) = Strom(A) x Spannung(V). Beispiel mit 48V Batterie: 60A x 48V = 2880W (2,9kW).
      includegenport: # GEN-Anschluss als Solar-Eingang verwenden, Wenn aktiviert, wird der GEN-Anschluss des Wechselrichters als zusätzlicher Solar-Eingang behandelt. Dadurch werden die Leistung und Energie, die über den GEN-Port eingespeist werden, zum Gesamt-Solarertrag addiert. (optional)
      firmware1098: # Firmware 1098 oder neuer (nur für HV Versionen), Nach einem Firmware-Update für Hochvolt-Wechselrichter haben einige Registerwerte ihren Skalierungsfaktor geändert. Wenn der Wechselrichter die Firmware-Version 1098 oder neuer hat, muss diese Option aktiviert werden, um korrekte Werte zu erhalten. (optional)`}
/>
...
<DeviceConfig
  ...
  advanced={`meters:
    - name: my_battery
      ...
      minsoc: 25 # Minimaler Ladestand (%), Untere Grenze beim Entladen der Batterie im normalen Betrieb (optional)
      maxsoc: 95 # Maximaler Ladestand (%), Oberes Limit beim Laden der Batterie aus dem Netz (optional)
      storageunit: 1 # Nummer des Batteriespeichers, [1, 2] (optional)
      maxchargepower: # Maximale Ladeleistung (W), Für erzwungenes Laden des Speichers. (optional)
      maxdischargepower: # Maximale Entladeleistung (W), Maximale Entladeleistung des Speichers. (optional)
      includegenport: # GEN-Anschluss als Solar-Eingang verwenden, Wenn aktiviert, wird der GEN-Anschluss des Wechselrichters als zusätzlicher Solar-Eingang behandelt. Dadurch werden die Leistung und Energie, die über den GEN-Port eingespeist werden, zum Gesamt-Solarertrag addiert. (optional)
      firmware1098: # Firmware 1098 oder neuer (nur für HV Versionen), Nach einem Firmware-Update für Hochvolt-Wechselrichter haben einige Registerwerte ihren Skalierungsfaktor geändert. Wenn der Wechselrichter die Firmware-Version 1098 oder neuer hat, muss diese Option aktiviert werden, um korrekte Werte zu erhalten. (optional)`}
...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      usage: grid
      host: 192.0.2.2 # IP-Adresse oder Hostname
      token: # Token, Ab Envoy Firmware D7.x.xxx notwendig. Token ist ein Jahr gültig. Anleitung (Obtaining a token via web UI): [enphase.com](https://enphase.com/download/accessing-iq-gateway-local-apis-or-local-ui-token-based-authentication) (optional)`}
  advanced={`meters:
    - name: my_grid
      type: template
      template: enphase
...
<DeviceConfig
  code={`meters:
    - name: my_battery
      ...
      host:  # Hostname
      port: 502 # Port
      minsoc: 10 # Minimaler Ladestand (%), Untere Grenze beim Entladen der Batterie im normalen Betrieb
      maxsoc: 100 # Maximaler Ladestand (%), Oberes Limit beim Laden der Batterie aus dem Netz
      maxchargepower: 1200 # Maximale Ladeleistung (W), Für erzwungenes Laden des Speichers.
...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      user: # Benutzerkonto, bspw. E-Mail Adresse, User Id, etc.
      password: # Passwort, Zugangspasswort des Dienstes
      ain: 307788992233 # Aktoridentifikationsnummer (AIN), Ist auf dem Typenschild auf der Geräterückseite aufgedruckt.
  ...
  advanced={`meters:
    - name: my_grid
      ...
      ain: 307788992233 # Aktoridentifikationsnummer (AIN), Ist auf dem Typenschild auf der Geräterückseite aufgedruckt.
      uri: https://fritz.box # URI, HTTP(S) Adresse (optional)
      unit: # Einheit, Index der Einheit für Geräte mit mehreren Einheiten (optional)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/meter  -->
...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      password: # Passwort (für aktive Batteriesteuerung), Zugangspasswort des Dienstes (optional)`}
  advanced={`meters:
    - name: my_grid
      type: template
      template: fronius-solarapi-v1
...
### ET/EH/BT/BH Hybrid Inverter (WiFi)
...
<DeviceConfig
  ...
  advanced={`meters:
    - name: my_battery
      ...
      maxsoc: 95 # Maximaler Ladestand (%), Oberes Limit beim Laden der Batterie aus dem Netz (optional)
      storageunit: 1 # Nummer des Batteriespeichers, optional
      forceaccharging: false # Wechselrichterkaskade, AC-Laden bleibt aktiv zum Laden des Speichers aus anderen AC Quellen für Wechselrichterkaskaden. Verhindert u.U. den Standby mit älteren Firmware-Versionen. (optional)`}
/>
...
### SUN2000 PV-Wechselrichter (ohne Batteriespeicher)
...
<DeviceConfig
  code={`meters:
    - name: my_pv
      ...
      password: admin # Passwort, Zugangspasswort des Dienstes (optional)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/meter  -->
...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      usage: grid
      host: 192.0.2.2 # IP-Adresse oder Hostname
      series: GridNet # L1 / Einphasig-Serie, IoTaWatt Serienname (Watt). Verfügbare Serien auf der IoTaWatt-Statusseite prüfen.`}
  advanced={`meters:
    - name: my_grid
      type: template
      template: iotawatt
      ...
      host: 192.0.2.2 # IP-Adresse oder Hostname
      series: GridNet # L1 / Einphasig-Serie, IoTaWatt Serienname (Watt). Verfügbare Serien auf der IoTaWatt-Statusseite prüfen.
      seriesL2: # L2 Serie (Dreiphasig), Serienname für Phase L2. Für einphasig leer lassen. (optional)
      seriesL3: # L3 Serie (Dreiphasig), Serienname für Phase L3. Für einphasig leer lassen. (optional)
...
<DeviceConfig
  code={`meters:
    - name: my_battery
      ...
      host:  # Hostname
      port: 502 # Port
      capacity: 2.12 # Akkukapazität (kWh), Basisgerät 2.12 kWh, jede Erweiterung zzgl. 2.12 kWh (optional)
...
<DeviceConfig
  code={`meters:
    - name: my_battery
      ...
      host:  # Hostname
      port: 502 # Port
      capacity: 5.12 # Akkukapazität (kWh), optional
      minsoc: 11 # Minimaler Ladestand (%), Untere Grenze beim Entladen der Batterie im normalen Betrieb (optional)
      maxsoc: 100 # Maximaler Ladestand (%), Oberes Limit beim Laden der Batterie aus dem Netz (optional)`}
  advanced={`meters:
    - name: my_battery
...
<DeviceConfig
  ...
  advanced={`meters:
    - name: my_battery
      ...
      minsoc: 11 # Minimaler Ladestand (%), Untere Grenze beim Entladen der Batterie im normalen Betrieb (optional)
      maxsoc: 100 # Maximaler Ladestand (%), Oberes Limit beim Laden der Batterie aus dem Netz (optional)
      maxchargepower: 2200 # Maximale Ladeleistung (W), Für erzwungenes Laden des Speichers. (optional)
      maxdischargepower: # Maximale Entladeleistung (W), Maximale Entladeleistung des Speichers. (optional)
      work_mode_normal: 1 # Work mode für Normal-Modus, 0=manuell, 1=Eigenverbrauch, 2=AI-Optimierung. (optional)`}
/>
...
<DeviceConfig
  ...
  advanced={`meters:
    - name: my_battery
      ...
      maxdischargepower: 4200 # Maximale Entladeleistung (W), Maximale Entladeleistung des Speichers. (optional)
      watchdog: 60s # Watchdog, optional`}
/>
...
<DeviceConfig
  code={`meters:
    - name: my_battery
      ...
      host:  # Hostname
      port: 502 # Port
      maxchargepower: 4200 # Maximale Ladeleistung (W), Für erzwungenes Laden des Speichers.
      capacity: 50 # Akkukapazität (kWh), optional
      minsoc: 25 # Minimaler Ladestand (%), Untere Grenze beim Entladen der Batterie im normalen Betrieb (optional)
      maxsoc: 95 # Maximaler Ladestand (%), Oberes Limit beim Laden der Batterie aus dem Netz (optional)`}
  advanced={`meters:
...
Zu den Details wie man den Wechselrichter verbindet siehe die Sofar Solar Installations Anleitung von [homeassistant-solax-modbus.readthedocs.io](https://homeassistant-solax-modbus.readthedocs.io/en/latest/sofar-installation/).
...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      usage: grid
      tokenid: # SolaxCloud TokenID, [solaxcloud.com](https://www.solaxcloud.com/) -> Support -> Drittanbieter-Ökosystem (alte Website) oder Dienst -> API (neue Website), den Wert von \`tokenID\` hier eintragen (Beispiel: 20241028488283838)
      serial: # Seriennummer, [solaxcloud.com](https://www.solaxcloud.com/) -> Gerät -> Wechselrichter (neue Website) oder Support (alte Website), Wert von Registrierungsnummer hier eintragen`}
  advanced={`meters:
    - name: my_grid
      type: template
      template: solax-hybrid-cloud
...
<DeviceConfig
  ...
  advanced={`meters:
    - name: my_battery
      ...
      maxchargepower: 1200 # Maximale Ladeleistung (W), Für erzwungenes Laden des Speichers. (optional)
      maxdischargepower: 800 # Maximale Entladeleistung (W), Maximale Entladeleistung des Speichers. (optional)`}
/>
...
<DeviceConfig
  code={`meters:
    - name: my_grid
      ...
      usage: grid
      host: 192.0.2.2 # IP-Adresse oder Hostname
      uuid: # Die vzlogger Kanal uuid für die Leistung
  ...
  advanced={`meters:
    - name: my_grid
      ...
      uuid: # Die vzlogger Kanal uuid für die Leistung
      port: 8081 # Port, optional
      scale: 1 # Skalierungsfaktor Leistung, Multipliziere Leistungs-Rohwert mit diesem Faktor (optional)
      energyuuid: # Die vzlogger Kanal uuid für den Zählerstand, Die vzlogger Kanal uuid für den Zählerstand (OBIS Code 1.8.0, Strombezug) (optional)
      energyscale: 0.001 # Skalierungsfaktor Energie, Multipliziere Energie-Rohwert mit diesem Faktor (optional)
...
<DeviceConfig
  ...
  advanced={`meters:
    - name: my_pv
      ...
      ain: 307788992233 # Aktoridentifikationsnummer (AIN), Ist auf dem Typenschild auf der Geräterückseite aufgedruckt.
      uri: https://fritz.box # URI, HTTP(S) Adresse (optional)
      firmware82: # Neue REST-API verwenden (FritzOS 8.2+), Verwende die neue REST-API für FritzOS ab Version 8.2 (optional)
      unit: # Einheit, Index der Einheit für Geräte mit mehreren Einheiten (nur REST-API) (optional)`}
/>
...
<DeviceConfig
  code={`meters:
    - name: my_pv
      ...
      usage: pv
      host: 192.0.2.2 # IP-Adresse oder Hostname
      token: # Token, API-Token, nur erforderlich wenn ein Token auf Gerät dem gesetzt ist (Experte -> REST API aktivieren -> Token) (optional)`}
/>
