...
### Energy Meter (einfaches KEM, nicht das Smart Meter KSEM, s. unten)
<Tabs>
<TabItem value="grid" label="Netz" default>
<DeviceFeatures features="" />

Netzeinspeisung bisher nicht getestet. (int16 statt uint16 wählen? oder gibt es für die Einspeiserichtung eigene Register?).
Die Baud Rate und die Slave ID (Standard ist 1, im Beispiel auf 21 geändert) kann man am Gerät einstellen, auch wenn das nicht dokumentiert ist.
...
meters:
  - name: my_grid
    ...
    power: # power (W)
      ...
      - source: modbus
        id: 21
        uri: 192.168.0.2:502 # hostname
        register:
          address: 18
          type: holding # Read an input register
          decode: uint16
        scale: 0.1
      - source: modbus
        id: 21
