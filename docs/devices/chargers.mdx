...
## EVBox Livo
Es ist möglich die Wallbox EVBox Livo mit eebus mit evcc zu verbinden. 
Dazu folgenden steps: 
1. Starten Sie EVCC mit der EEBus-Konfiguration (Zertifikate, SKI usw.).
2. Verbinden Sie sich über die EV Install App mit der Wallbox und wählen Sie den Energie-Manager aus. Die EVCC sollte durch das Discovery-Protokoll auftauchen.
3. Drücken Sie auf Verbinden und suchen Sie den SKI in den Logs.
4. Passen Sie den SKI in den EVCC Ladegeräten an.
5. Starten Sie EVCC neu.
Achtung: Es gibt Probleme mit Home Assistant und IPv6. [hier](https://github.com/evcc-io/hassio-addon/issues/99)
