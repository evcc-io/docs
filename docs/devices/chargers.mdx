...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      user: # Benutzerkonto, bspw. E-Mail Adresse, User Id, etc. (optional)
      password: # Passwort, Bei führenden Nullen bitte in einfache Hochkommata setzen (optional)`} advanced={`chargers:
    - name: my_charger
      type: template
      template: zaptec
      id: # ID, Wallbox ID (optional)
...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      enabled: false # Ladebereit, optional`} advanced={`chargers:
    - name: my_charger
      type: template
      template: demo-charger
      status: A # Ladezustand, [A, B, C]
...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      power: 0 # Leistung (W), optional
      enabled: true # Bereit zum Heizen, optional`} advanced={`chargers:
    - name: my_charger
      type: template
      template: demo-heatpump
      operationMode: heating # Betriebszustand, [standby, heating]
...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      lon: 9.1275 # Längengrad, optional`} advanced={`chargers:
    - name: my_charger
      type: template
      template: vehicle-api
      geofence_enabled: false # Geofencing aktivieren, optional
...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      uri: http://192.168.0.10:8080/semp # SEMP Basis-URL, HTTP(S) Adresse`} advanced={`chargers:
    - name: my_charger
      type: template
      template: semp-charger
...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      firmware: <1.1.3 # Firmware version. Wähle '>=1.1.3' für Word-Swap, [<1.1.3, >=1.1.3] (optional)`} advanced={`chargers:
    - name: my_charger
      type: template
      template: lambda-zewotherm
      host: 192.0.2.2 # IP-Adresse oder Hostname
...
<DeviceConfig code={`chargers:
    - name: my_charger
      ...
      redirecturi: https://evcc.example.org/providerauth/callback # Redirect URI, Redirect-URI der evcc-Instanz. Muss mit der Redirect URI übereinstimmen, die Viessmann Developer Portal konfiguriert ist.
