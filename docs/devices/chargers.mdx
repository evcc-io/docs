...
Die "Active load balancing" Lizenz wird benötigt um die Wallbox via Modbus extern zu steuern. In den Einstellungen muss "Active Load Balancing" aktiviert und "Steckdose" als Data Source ausgewählt werden. Soll sowohl mit einer als auch mit drai phasen geladen werden, muss der "sichere Ladestrom" auf 6A eingestellt werden. Außerdem ist drauf zu achten, dass der Ladestrom nicht über die "Eve Connect" App begrenzt ist. Es wird empfohlen "ValidityTime" (Menu "TCP/IP EMS") auf 300s einzustellen. Falls die "Double"-Box verwendet wird müssen beide Ladepunkte getrennt voneinander hinzugefügt werden. Der erste Port (oder einzelne Port) ist unter ID 1 zugänglich, der zweite unter ID 2.
