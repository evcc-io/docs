...
## EM2GO Home, Pro Power, OCPP/ONC

<DeviceFeatures features="mA,sponsorfree" />

Aktuelle Firmware mit Modbus-Unterstützung notwendig (Home: E3C_V1.1, Pro Power: 1.01 bzw. OCPP/ONC: 3.15)
...
Aktuelle Firmware erforderlich.

Um den Enector mit evcc zu betreiben sind hardwareseitig folgende Schritte zu unternehmen:

1. Modbus-Kabel direkt vom Enector (nicht über KSEM Smart Meter) mit RS485-Adapter oder RS485 TCP/IP-Gateway verbinden. Die Anschlüsse sind wie in der Beschreibung von Kostal gekreuzt (A->B; B->A).

2. Dip Schalter auf Bank 1 (S1) so setzen:
...
3. Verbindungparameter TCP/IP-Gateway:
