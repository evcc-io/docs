...
Werden die Werte für `minSoc` und `targetSoc` im WebUI geändert, werden diese dauerhaft beim Fahrzeug gespeichert. Auf die EInstellungen in der evcc.yaml wird dann nur noch bei einem Neustart zurückgegriffen.
