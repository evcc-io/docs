...
- Prüfe die Installation und öffne einen Browser und gibt die folgende URL ein: `http://localhost:7070`. Die evcc Oberfläche sollte im Demo Modus zu sehen sein.
  ...
  :::note
  Sollte der Browser auf einem anderen Computer laufen, ist im Folgenden `localhost` durch die IP-Adresse unseres Rechners zu ersetzen. In Heimnetzen startet die IP-Adresse oft mit `192.168.xx.xx`. Sie kann wie folgt in Erfahrung gebracht werden: 
  ```sh
