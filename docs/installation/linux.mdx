...
:::note
Sollte eine serielle Schnitstelle verwendet werden kann es sein das dierser Befehl ausgeführt werden muss `sudo usermod -a -G plugdev evcc`. Dieser Befehl fügt den Benutzer evcc zur Gruppe plugdev hinzu. Die Gruppe plugdev dient dazu, Zugriffsrechte für Plug-in-Geräte (wie USB, serielle Schnittstelle usw.) zu erteilen, ohne dass Root-Rechte erforderlich sind.
::::
...
Anmerkung fall die installation auf einem modernen Raspberry Pi OS geschieht ist der Befehl 'sudo usermod -a -G plugdev evcc' notwendig fall eine serielle Schnitstelle zum einsatz kommt 
