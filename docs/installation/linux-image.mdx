...
Gehe zu den **[evcc Linux Images](https://github.com/evcc-io/images/releases)** und lade die neueste Version für Raspberry Pi herunter (`armbian_evcc-{version}_rpi.img.zip`).
...
| Service     | URL                                                      |
| ----------- | -------------------------------------------------------- |
| evcc Web UI | `https://evcc.local/`                                    |
| OCPP Server | `wss://evcc.local:8887/<station-id>`                     |
| Cockpit     | `https://evcc.local:9090/` (initiales Passwort: `admin`) |
| SSH         | `ssh admin@evcc.local` (initiales Passwort: `admin`)     |

Die Services verwenden selbst-signierte Zertifikate.
Die Zertifikatwarnung im Browser muss akzeptiert werden, oder die Zertifikatsprüfung in der OCPP-Konfiguration deiner Wallbox muss deaktiviert werden.
Bei Kompatibilitätsproblemen kannst du die unverschlüsselten Alternativen verwenden:

| Service     | URL                                 |
| ----------- | ----------------------------------- |
| evcc Web UI | `http://evcc.local:7070/`           |
| OCPP Server | `ws://evcc.local:8886/<station-id>` |
