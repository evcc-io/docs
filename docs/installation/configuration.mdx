...
Diese Konfigurations-Anleitung beschreibt unterschiedliche Wege eine funktionierende Konfigurationsdatei (`evcc.yaml`) zu erstellen.

## Mit Assistenten

Dieser Teil der Anleitung richtet sich an Benutzer, die evcc als Linux/Mac oder Windows Installation nutzen.
Es wird vorausgesetzt, dass evcc bereits auf deinem System installiert ist.
...
:::note
Fortgeschrittene Anwender (z.B. mit evcc Erfahrung oder technischem Know-How) können alternativ folgenden Aufruf verwenden:

```sh
evcc configure --advanced
```

In diesem Modus werden weitere Einstellungsmöglichkeiten abgefragt.
:::
