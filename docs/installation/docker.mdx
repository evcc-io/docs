...
Installiere den Docker Container:

<Tabs groupId="docker">
  <TabItem value="default" label="Standard" default>

  ```sh
  docker run -d --name evcc -v $(pwd)/evcc.yaml:/etc/evcc.yaml -p 7070:7070 andig/evcc:latest
  ```
  </TabItem>
  ...
  ```sh
  docker run -d --name evcc -v $(pwd)/evcc.yaml:/etc/evcc.yaml -v /etc/machine-id:/etc/machine-id -v /var/lib/dbus/machine-id:/var/lib/dbus/machine-id --network host andig/evcc:latest
  ```
  </TabItem>
  ...
  ```sh
  docker run -d --name evcc -v $(pwd)/evcc.yaml:/etc/evcc.yaml -p 7070:7070 -p 7090:7090/udp andig/evcc:latest
  ```
  </TabItem>
</Tabs>
