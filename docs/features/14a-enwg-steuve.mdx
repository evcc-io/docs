...
Dies betrifft z. B. Wärmepumpen oder Batteriespeicher, die als Zähler mit Schaltausgang konfiguriert sind.

### Voraussetzungen

- Der Verbraucher muss als Zähler mit Schaltausgang (`switch`) in evcc konfiguriert sein
- HEMS-Konfiguration (Relais oder EEBus) muss eingerichtet sein
- Der Zähler muss dem `lpc` Stromkreis zugeordnet werden
...
meters:
  - name: heatpump
    ...
    template: eebus-mcp
    usage: charge
    # Weitere Zähler-Konfiguration ...

circuits:
  - name: lpc
    meter: heatpump # Zähler zuordnen

hems:
  type: eebus # oder relay
  # Weitere HEMS-Konfiguration ...
```

### Funktionsweise

Bei aktivem Reduzierungssignal regelt evcc automatisch:

1. **Ladepunkte**: Leistung wird entsprechend dem Limit gedrosselt
2. **Schaltbare Zähler**: Werden bei Bedarf ab- und zugeschaltet, um das Gesamtlimit einzuhalten

Die Steuerung berücksichtigt dabei die aktuelle Leistungsaufnahme aller dem `lpc` Stromkreis zugeordneten Verbraucher.
Reicht die Drosselung der Ladepunkte nicht aus, werden weitere Verbraucher abgeschaltet.

:::note EEBus Verbraucher
Diese Funktion ist derzeit nur für EEBus Verbraucher verfügbar.
Bei einigen Geräten kann die Leistung auch stufenlos gedrosselt werden.
Dies ermöglicht eine präzisere Regelung als das reine Ein-/Ausschalten.
Unterstützung für weitere Zähler wird in Zukunft hinzugefügt.
:::
