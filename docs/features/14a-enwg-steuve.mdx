...
Wenn die Steuerbox oder ein Gateway den das Leistungslimit vis Modbus bereitstellt:
...
hems:
  ...
  limit: 
    source: modbus  # Details dar Parameter siehe evcc-modbus Dokumntation 
    uri: 192.168.179.200:4703  # Beispiel für ein am 2. S-Zähler angeschlossenem Relais einer cfos Power Brain solar
    id: 3  # Modbus Slave-ID des Geräts 
    timeout: 5s  
    register:
      type: holding  
      decode: uint16  
