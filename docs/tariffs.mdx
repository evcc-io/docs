...
<DeviceConfig code={`tariffs:
    grid:
      type: template
      template: stekker
      region: BE # Region, [DE-LU, EE, LT, LV, AT, BE, FR, GER, NL, PL, DK1, DK2, FI, NO1, NO2, NO3, NO4, NO5, SE1, SE2, SE3, SE4, TEL, SYS, CH, RO, PT, RS, SI, SK, HU, CZ, HR] (optional)`} advanced={`tariffs:
    grid:
      type: template
      template: stekker
      region: BE # Region, [DE-LU, EE, LT, LV, AT, BE, FR, GER, NL, PL, DK1, DK2, FI, NO1, NO2, NO3, NO4, NO5, SE1, SE2, SE3, SE4, TEL, SYS, CH, RO, PT, RS, SI, SK, HU, CZ, HR] (optional)
      charges: # Aufschlag, Zusätzlicher fester Aufschlag pro kWh (z.B. 0.05 für 5 Cent) (optional)
      tax: # Steuer, Zusätzlicher prozentualer Aufschlag (z.B. 0.2 für 20%) (optional)
      formula: math.Max((price + charges) * (1 + tax), 0.0) # Formel, Individuelle Formel zur Berechnung des Preises (optional)`} />
...
<DeviceConfig code={`tariffs:
    co2:
      ...
      variation: 0.4 # Variationsfaktor, Variationsfaktor zur Simulation täglicher Schwankungen (0.4 = 40%) (optional)`} advanced={`tariffs:
    co2:
      type: template
      template: demo-co2-forecast
      base: 350 # Durchschnittliche CO₂-Emissionen (g/kWh), optional
...
<DeviceConfig code={`tariffs:
    ...
    solar:
      - type: template
        ...
        algorithm: mosmix # Prognosealgorithmus (mosmix, own-v1 oder clearsky), [mosmix, own-v1, clearsky] (optional)
        interval: 1h # Intervall, optional`} />
