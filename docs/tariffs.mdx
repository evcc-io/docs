...
**Update-Häufigkeit und Caching**

Tarif-Plugins werden einmal pro Stunde aktualisiert.
Die Update-Häufigkeit der integrierten Tarif-Anbieter hängt vom jeweiligen Anbieter und dessen Ratelimit ab.
Bei einigen Anbietern kann die Häufigkeit über den `interval` Parameter angepasst werden.

evcc speichert abgerufene Tarifdaten automatisch zwischen.
Bei einem Neustart werden gecachte Daten verwendet, sofern diese noch gültig sind.
Dies verhindert, dass API-Ratelimits bei häufigen Neustarts überschritten werden.
