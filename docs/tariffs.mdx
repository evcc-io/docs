...
### CKW Netz Home/Business dynamic
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: ckw
      tariff: # Tarifname, home_dynamic (unter 50 MWh/Jahr) oder business_dynamic (über 50 MWh/Jahr) [home_dynamic, business_dynamic]`}
  advanced={`tariffs:
    grid:
      type: template
      template: ckw
      tariff: # Tarifname, home_dynamic (unter 50 MWh/Jahr) oder business_dynamic (über 50 MWh/Jahr) [home_dynamic, business_dynamic]
      charges: # Aufschlag, Zusätzlicher fester Aufschlag pro kWh (optional)
      tax: 0.081 # Steuer, Zusätzlicher prozentualer Aufschlag (z.B. 0.2 für 20%) (optional)
      formula: math.Max((price + charges) * (1 + tax), 0.0) # Formel, Individuelle Formel zur Berechnung des Preises (optional)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: ekz
      tariff_name: 400D # Desired tariff (400D for dynamic)., [400D, 400F, 400ST, 400WP, 400L, 400LS, 16L, 16LS]`}
  advanced={`tariffs:
    grid:
      type: template
      template: ekz
...
<DeviceFeatures features="Deutschland,Luxemburg,Österreich,Frankreich,Niederlande,Belgien,Polen,Dänemark" />
...
### Energy-Charts Börsenstrompreis

<DeviceFeatures features="EU" />

Day-ahead Energiepreise an der Börse. Bereitgestellt von Fraunhofer ISE. Kann ohne vorherige Anmeldung auf [api.energy-charts.info](https://api.energy-charts.info/) abgerufen werden. Nutzbar u.a. für dynamische Stromtarife, wo der Anbieter bis dato auf der Kundenschnittstelle noch kein Preis-Vorhersagen anbietet.
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: energy-charts-api
      bzn: DE-LU # Gebotszone, siehe [api.energy-charts.info](https://api.energy-charts.info/#/prices/day_ahead_price_price_get) [AT, BE, CH, CZ, DE-LU, DE-AT-LU, DK1, DK2, FR, HU, IT-NORTH, NL, NO2, PL, SE4, SI]`}
  advanced={`tariffs:
    grid:
      type: template
      template: energy-charts-api
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: entsoe
      securitytoken: # Security token, Registrierung und anschließende Helpdesk-Anfrage erforderlich. Details zum Ablauf gibts hier [transparency.entsoe.eu](https://transparency.entsoe.eu/content/static_content/Static%20content/web%20api/Guide.html#_authentication_and_authorisation) (optional)
      domain: BZN|DE-LU # Domain, siehe [transparency.entsoe.eu](https://transparency.entsoe.eu/content/static_content/Static%20content/web%20api/Guide.html#_areas) (optional)`}
  advanced={`tariffs:
    grid:
      type: template
      template: entsoe
...
### EPEX Predictor Predicted EPEX spot prices

<DeviceFeatures features="Deutschland,Österreich,Niederlande,Belgien,Schweden,Dänemark" />

EPEX Spot Preisvorhersagen von epexpredictor.batzill.com
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: epex-predictor
      region: DE # Region, [DE, AT, NL, BE, SE1, SE2, SE3, SE4, DK1, DK2]`}
  advanced={`tariffs:
    grid:
      type: template
      template: epex-predictor
      region: DE # Region, [DE, AT, NL, BE, SE1, SE2, SE3, SE4, DK1, DK2]
      uri: https://epexpredictor.batzill.com # API-Endpunkt (für selbst gehostete Instanzen), HTTP(S) Adresse (optional)
      charges: # Aufschlag, Zusätzlicher fester Aufschlag pro kWh (optional)
      tax: # Steuer, Zusätzlicher prozentualer Aufschlag (z.B. 0.2 für 20%) (optional)
      formula: math.Max((price + charges) * (1 + tax), 0.0) # Formel, Individuelle Formel zur Berechnung des Preises (optional)
      average: # Stündliche Durchschnittskosten verwenden, optional`}
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      provider: # Energieversorger, [anwb-energie, atoom-alliantie, budget-energie, coolblue-energie, easyenergy, eneco, engie, energie-vanons, energyzero, frank-energie, frank-energie-slim, hegg, innova, nextenergy, samsam, tibber, vandebron, zonneplan]
      price-interval: quarterly # Preisintervall, [hourly, quarterly]`}
  advanced={`tariffs:
    grid:
      type: template
      template: epexprijzen-nl
...
### EWS Elektrizitätswerke Schönau Ökostrom Dynamisch
...
Den API-Key bekommst du im Octopus Portal: [octopus.energy](https://octopus.energy/dashboard/new/accounts/personal-details/api-access)
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: octopus-de
      email: user@example.com # E-Mail-Adresse, Die E-Mail-Adresse Ihres Octopus Energy Kontos.
      password: secret # Passwort, Das Passwort Ihres Octopus Energy Kontos.
      accountNumber: A-XX345678 # Kundennummer, Ihre Octopus Energy Kundennummer (z.B. A-12345678).`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      region: # Region, Die DNO-Region, in der Sie sich befinden. Weitere Informationen: [energy-stats.uk](https://www.energy-stats.uk/dno-region-codes-explained/) [A, B, C, D, E, F, G, H, J, K, L, M, N, P]
...
<DeviceFeatures features="Polen" />

Hol dir deinen API-Token aus der Pstryk App.
...
<DeviceConfig
  code={`tariffs:
    grid:
      type: template
      template: pstryk
      token: # API-Token (z.B. sk-...).
      plan: pricing # Tarifquelle, [pricing, prosumer-pricing]`}
  advanced={`tariffs:
    grid:
      type: template
      template: pstryk
...
Preisdaten von [mercatoelettrico.org](https://www.mercatoelettrico.org/it/). Wird oft zur Einspeisung ins Netz verwendet.
...
<DeviceFeatures features="Spanien" />

Spain PVPC tariff extracted from [api.esios.ree.es](https://api.esios.ree.es). It is possible to create grid and feed-in tariffs. You need a token in order to be able to use the API
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      contractid: ffffffff-4444-6666-2222-aaaaaabbbbbb # Vertragsnummer, Die Vertragsnummer bekommst du im Kundenportal [i.spottyenergie.at](https://i.spottyenergie.at/)
      pricetype: CONSUMPTION # Preistyp, Preistyp, entweder Börsenpreis, Verbrauchspreis oder Einspeisevergütung (falls vereinbart), siehe [spottyenergie.at](https://www.spottyenergie.at/blog/energie-smart-produzieren) [MARKET, CONSUMPTION, GENERATION]`}
  advanced={`tariffs:
    grid:
      type: template
      template: spottyenergy
...
### stroomprijsprognose.nl

<DeviceFeatures features="Deutschland,Niederlande,Belgien,Dänemark,Frankreich" />

Day-Ahead-Strompreise plus 4 Tage Prognose [stroomprijsprognose.nl](https://stroomprijsprognose.nl/de/evcc-strompreis-co2-prognose/)
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      region: DE # Region, [DE, NL, BE, DK1, DK2, FR]
      currency: EUR # Währung, [EUR, DKK]
      horizon: 120 # Prognosehorizont (h)`}
  advanced={`tariffs:
    grid:
      type: template
      template: stroomprijsprognose
...
Zeitabhängige Strompreise mit verschiedenen Preiszonen für Nachttarife, Wochenendtarife oder saisonale Tarife. Wenn sich Zonen überschneiden, gilt die zuletzt zutreffende Zone.
...
<DeviceConfig
  code={`tariffs:
    grid:
      ...
      price: # Standardpreis, Wenn keine Zone zutrifft
      zones: # Preiszonen`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      token: # Token
      zone: DE # Zone, siehe [api.electricitymap.org](https://api.electricitymap.org/v3/zones)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceFeatures features="Finnland" />

CO₂-Daten für das finnische nationale Netz von [Fingrid OpenData](https://www.fingrid.fi/en/electricity-market-information/real-time-co2-emissions-estimate/). Erhalte deinen API-Schlüssel, indem du ein Konto unter [https://data.fingrid.fi/instructions](https://data.fingrid.fi/instructions) registrierst.
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      apiKey: # API Key, Erstellen Sie Ihren API-Schlüssel, indem Sie ein Konto unter [https://data.fingrid.fi/instructions](https://data.fingrid.fi/instructions) registrieren, einen neuen API-Schlüssel erstellen und diesen hier eingeben.`}
  advanced={`tariffs:
    co2:
      type: template
      template: fingrid-co2
...
Regionale Emissionsdaten von [gruenstromindex.de](https://gruenstromindex.de)
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      zip: # Postleitzahl
      token: # Token, Token für den Zugriff auf die API von [console.corrently.io](https://console.corrently.io/) (optional)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    co2:
      type: template
      template: ned
      apiKey: # API Key, Erstelle eine App in [ned.nl](https://ned.nl/nl/user/register) und kopiere den Key`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
...
<DeviceConfig
  code={`tariffs:
    co2:
      type: template
      template: ngeso
      region: 1 # Region, Ungenauer als die Verwendung eines Postleitzahl. Siehe [carbon-intensity.github.io](https://carbon-intensity.github.io/api-definitions/#region-list) (optional)
...
### stroomprijsprognose.nl/co2

<DeviceFeatures features="Deutschland,Niederlande,Belgien,Dänemark" />

5 Tage Prognose [stroomprijsprognose.nl](https://stroomprijsprognose.nl/de/evcc-strompreis-co2-prognose/)
...
<DeviceConfig
  code={`tariffs:
    co2:
      ...
      region: DE # Region, [DE, NL, BE, DK1, DK2]
      horizon: 120 # Prognosehorizont (h)`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
  ...
  code={`tariffs:
    solar:
      - type: template
        ...
        az: 0 # Azimut, Ausrichtung der PV-Module in Grad. -180 = Norden, -90 = Osten, 0 = Süden, 90 = Westen, 180 = Norden
  ...
  advanced={`tariffs:
    solar:
      - type: template
        ...
        dec: 25 # Neigung, 0 = horizontal, 90 = vertikal
        kwp: 9.8 # Maximale Generatorleistung (kWp)
        horizon: 0,0,15,30,45,60,60,60,45,30,15,0 # Horizont, Simuliert Verschattung durch Gelände, [doc.forecast.solar](https://doc.forecast.solar/horizon) (optional)
  ...
  advanced={`tariffs:
    solar:
      - type: template
        ...
        rossmodel: 0.0342 # Kühlung [Ross-Modell], Gut Gekühlt (0.0200), Freistehend (0.0208), Flach auf Dach (0.0260), Nicht So Gut Gekühlt (0.0342), Transparentes PV (0.0455), Fassadenintegriert (0.0538), Auf Schrägdach (0.0563) [sciencedirect.com](https://www.sciencedirect.com/science/article/pii/S2352484722024805) (optional)
        interval: 1h # Intervall, optional`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
  ...
  advanced={`tariffs:
    solar:
      - type: template
        ...
        interval: 24h # Intervall, optional`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
  ...
  advanced={`tariffs:
    solar:
      - type: template
        ...
        algorithm: # Prognosealgorithmus (mosmix, own-v1 oder clearsky), [mosmix, own-v1, clearsky] (optional)
        interval: 1h # Intervall, optional`}
/>
<!-- AUTO-GENERATED FROM TEMPLATE - PLEASE EDIT HERE https://github.com/evcc-io/evcc/tree/master/templates/definition/tariff  -->
